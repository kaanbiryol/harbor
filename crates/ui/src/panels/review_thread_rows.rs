...
pub(crate) fn render_review_thread_row(state: ReviewThreadRowRenderState<'_>) -> AnyElement {
    ...
    let is_inactive = thread.state != ReviewThreadState::Unresolved;
    let row_border_color = if is_inactive {
        color::border_subtle()
    } else {
        color::border()
    };
    let row_bg_color = if is_inactive {
        color::content_background()
    } else {
        color::elevated_background()
    };
    let metadata_color = if is_inactive {
        color::text_disabled()
    } else {
        color::text_muted()
    };
    let comment_color = if is_inactive {
        color::text_muted()
    } else {
            ...
            div()
                ...
                .child(
                    div()
                        ...
                        .child(render_review_thread_state_indicator(
                            label,
                            tone,
                            is_inactive,
                        )),
                ...
                .when_some(diff_preview, move |element, preview| {
                    element.child(div().px_3().pt_1().child(render_review_diff_preview(
                        preview,
                ...
                .when(ui_state.active_reply, {
                    ...
                    move |element| {
                        element.child(div().px_3().pt_1().pb_3().child(
...
fn render_review_thread_state_indicator(
    label: &'static str,
    tone: Tone,
    low_emphasis: bool,
) -> impl IntoElement {
    ...
    div()
        ...
        .text_xs()
        .font_medium()
        .text_color(if low_emphasis {
            color::text_muted()
        } else {
            tone_colors(tone).text
        })
        .child(div().size(px(6.)).rounded_full().bg(tone_colors(tone).text))
        .child(display_label)
}
