...
impl GitHubAuthSource {
    ...
    fn from_credential_value(value: &str) -> Option<Self> {
        match value.trim() {
    ...
    fn from_credential_bytes(bytes: Vec<u8>) -> std::result::Result<Option<Self>, String> {
...
mod tests {
    ...
    fn auth_source_round_trips_credential_values() {
        for source in [GitHubAuthSource::OAuth, GitHubAuthSource::GhCli] {
            assert_eq!(
                GitHubAuthSource::from_credential_value(source.credential_value()),
                Some(source)
            );
        }
    }
