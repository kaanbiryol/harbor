...
impl AppView {
    pub(super) fn spawn_pull_request_checks_loader(
        ...
        self.tasks.set_selected_pull_request_task(
            SelectedPullRequestTaskKind::Checks,
            cx.spawn({
                let repo = load.repo;
                        ...
                        move |view, cx| {
                            ...
                            match refresh.result {
                                Ok(check_runs) => {
                                    ...
                                    view.status =
                                        format!("Loaded {count} check runs for PR #{number}");
                                }
                                Err(error) => {
    ...
    pub(super) fn spawn_pull_request_workflows_loader(
        ...
        let detail_key = load.detail_key();
        self.tasks.set_selected_pull_request_task(
            SelectedPullRequestTaskKind::Workflows,
            cx.spawn({
                let repo = load.repo;
                let number = load.number;
                let head_sha = load.head_sha;
